...
Homework 3: Camera, Stereo and Motion Estimation
...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        # YOUR CODE HERE
        # 得到方程Ab = b的矩阵
        Ax = Ix[y-w:y-w+window_size, x-w:x-w+window_size].reshape(-1, 1)
        Ay = Iy[y-w:y-w+window_size, x-w:x-w+window_size].reshape(-1, 1)
        A = np.hstack((Ax, Ay))
        B = -It[y-w:y+w+1, x-w:x+w+1].reshape(-1, 1)
        # 通过最小二乘法计算速度矢量，看博客给的公式是最小二乘法解超定方程得到的公式，不太理解
        d = np.linalg.inv(A.T.dot(A)).dot(A.T).dot(B)
        # d此时是列向量，转换成行向量
...
def iterative_lucas_kanade(img1, img2, keypoints, window_size=9, num_iters=7, g=None):
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        # YOUR CODE HERE
        Ax = Ix[y1-w:y1-w+window_size, x1-w:x1-w+window_size].reshape(-1, 1)
        Ay = Iy[y1-w:y1-w+window_size, x1-w:x1-w+window_size].reshape(-1, 1)
        g0 = np.sum(Ax**2)
        g1 = np.sum(Ax * Ay)
        g2 = np.sum(Ay**2)
        G = np.array([[g0, g1],
        ...
        for k in range(num_iters):
            ...
            # YOUR CODE HERE
            Ik = (img1[y1 - w:y1 - w + window_size, x1 - w:x1 - w + window_size] -
                  img2[y2 - w:y2 - w + window_size, x2 - w:x2 - w + window_size]).reshape(-1, 1)
            bk = np.array([[np.sum(Ax * Ik)],
                           [np.sum(Ay * Ik)]])
            vk = np.dot(G_inv, bk)
            vk = vk.flatten()
            # END YOUR CODE
...
def pyramid_lucas_kanade(
    ...
    for L in range(level, -1, -1):
        ...
        d = iterative_lucas_kanade(
            pyramid1[L], pyramid2[L], P_L, window_size=window_size, num_iters=num_iters, g=g)
        if L != 0:
            g = scale * (g + d)
        # END YOUR CODE
